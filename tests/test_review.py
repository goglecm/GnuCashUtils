...
def _seed_proposals(state: StateRepository) -> list[Proposal]:
    proposals = [
        ...
        Proposal(
            ...
            suggested_splits=[
                Split(account_path="Expenses:Entertainment", amount=Decimal("15.99"))
            ],
            confidence=0.7,
            rationale="Keyword match",
            tx_date=date(2025, 1, 20),
            tx_amount=Decimal("15.99"),
        ...
        Proposal(
            ...
            suggested_splits=[
                Split(account_path="Expenses:Miscellaneous", amount=Decimal("42.00"))
            ],
            confidence=0.3,
...
class TestReviewQueueService:
    ...
    def test_get_account_paths_after_run(self, tmp_path: Path) -> None:
        ...
        state = StateRepository(tmp_path)
        state.save_metadata(
            "account_paths", {"paths": ["Expenses:Food", "Current Account", "Unspecified"]}
        )
        svc = ReviewQueueService(state)
    ...
    def test_submit_decision_saves_unenriched_when_enrichment_raises(self, tmp_path: Path) -> None:
        ...
        email_ev = EmailEvidence(
            ...
            body_snippet="Order for Widget",
            parsed_amounts=[Decimal("9.99")],
            full_body="Widget order",
        )
        prop = Proposal(
            proposal_id="p1",
            tx_id="tx1",
        ...
        with patch.object(
            svc, "_enrich_from_approved_evidence", side_effect=RuntimeError("Enrichment failed")
        ):
...
class TestReviewWebApp:
    ...
    def test_llm_button_hidden_when_no_description_emails_or_receipt(self, tmp_path: Path) -> None:
        ...
        state = StateRepository(tmp_path)
        state.save_metadata(
            "run_config", {"llm_mode": "online", "llm_endpoint": "http://x", "llm_model": "y"}
        )
        prop = Proposal(
            proposal_id="p1",
            tx_id="tx1",
    ...
    def test_llm_button_shown_when_llm_enabled_and_description_present(
        self, tmp_path: Path
    ) -> None:
    ...
    def test_review_page_200_with_empty_emails_when_display_build_raises(
        self, tmp_path: Path
    ) -> None:
        ...
        email_ev = EmailEvidence(
            ...
            sent_at=datetime(2025, 2, 1, tzinfo=timezone.utc),
            body_snippet="Body",
            parsed_amounts=[Decimal("10.00")],
            full_body="Full",
        )
        prop = Proposal(
            proposal_id="p1",
            tx_id="tx1",
            suggested_description="Pay",
            suggested_splits=[],
            confidence=0.8,
            rationale="r",
            tx_date=date(2025, 1, 15),
            tx_amount=Decimal("25.00"),
            original_description="TESCO",
            original_splits=[],
        ...
        client = app.test_client()
        with patch(
            "gnc_enrich.review.webapp.CategoryPredictor.get_emails_for_display",
    ...
    def test_llm_check_returns_200_with_null_extraction_when_serialize_raises(
        self, tmp_path: Path
    ) -> None:
        ...
        with patch("gnc_enrich.review.service.CategoryPredictor") as MockPredictor:
            ...
            client = app.test_client()
            with patch(
                "gnc_enrich.review.webapp._serialize_extraction_for_json",
    ...
    def test_run_llm_check_updates_proposal_llm_fields_preserves_ml(self, tmp_path: Path) -> None:
        ...
        prop = Proposal(
            ...
            suggested_description="ML description",
            suggested_splits=[
                Split(account_path="Expenses:Miscellaneous", amount=Decimal("25.00"))
            ],
            confidence=0.5,
            rationale="ML",
            tx_date=date(2025, 1, 15),
    ...
    def test_llm_check_api_returns_200_with_serialized_extraction(self, tmp_path: Path) -> None:
        ...
        with patch("gnc_enrich.review.service.CategoryPredictor") as MockPredictor:
            ...
            mock_instance.run_llm_check.return_value = {
                "extraction": {
                    "seller_name": "Tesco",
                    "items": [{"description": "Bread", "amount": Decimal("3.50")}],
                },
                "category": "Expenses:Food",
